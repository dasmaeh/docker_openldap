...
EOF
    ...
    # Switch to RFC2307bis schema
    # Credits: https://github.com/mesosphere/docker-containers/blob/master/openldap/rfc2307bis/startup.sh
    echo "Switching schema to RFC2307bis"
    cp /container/service/slapd/assets/config/bootstrap/rfc2307bis/rfc2307bis.* /etc/ldap/schema/
    mkdir -p /tmp/schema
    rm -f /etc/ldap/slapd.d/cn=config/cn=schema/*
    slaptest -f /container/service/slapd/assets/config/bootstrap/rfc2307bis/rfc2307bis.conf -F /tmp/schema
